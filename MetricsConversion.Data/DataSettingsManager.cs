...
namespace MetricsConversion.Data
{
    ...
    public partial class DataSettingsManager
    {
        ...
        public static DataSettings LoadSettings(string filePath = null, bool reloadSettings = false, IDefaultFileProvider fileProvider = null)
        {
            if (!reloadSettings && Singleton<DataSettings>.Instance != null)
                return Singleton<DataSettings>.Instance;

            fileProvider ??= CommonHelper.DefaultFileProvider;
            filePath ??= fileProvider.MapPath(DefaultDataSettingsDefaults.FilePath);

            //check whether file exists
            if (!fileProvider.FileExists(filePath))
            {
                //if not, try to parse the file that was used in previous simpleMarket versions
                filePath = fileProvider.MapPath(DefaultDataSettingsDefaults.ObsoleteFilePath);
                if (!fileProvider.FileExists(filePath))
        ...
        /// <param name="fileProvider">File provider</param>
        public static void SaveSettings(DataSettings settings, IDefaultFileProvider fileProvider = null)
        {
            Singleton<DataSettings>.Instance = settings ?? throw new ArgumentNullException(nameof(settings));

            fileProvider ??= CommonHelper.DefaultFileProvider;
            var filePath = fileProvider.MapPath(DefaultDataSettingsDefaults.FilePath);
