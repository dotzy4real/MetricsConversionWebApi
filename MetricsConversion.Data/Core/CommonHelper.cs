...
namespace MetricsConversion.Data.Core
{
    ...
    public partial class CommonHelper
    {
        ...
        {
            var isNumber = long.TryParse(PhoneNum, out long Phone);
            if (isNumber || PhoneNum.Length == 14 || PhoneNum.Length == 13 || PhoneNum.Length == 11 || PhoneNum.Length == 10)
            {
                string phoneNo = string.Empty;
                if (PhoneNum.StartsWith("+234") && PhoneNum.Length == 14)
                    phoneNo = PhoneNum.Replace("+234", "");
                else if (PhoneNum.StartsWith("234") && PhoneNum.Length == 13)
                    phoneNo = PhoneNum[3..];
        ...
        public static string RemoveHTMLTags(string stringContainingHTMLTags)
        {
        ...
        public static bool IsValidBetaStoreEmail(string email) {

            string betaPhoneNo = "8155546404";
            var phone = email.Split("@");

            if (email.StartsWith(betaPhoneNo))
                return false;

            if (!email.StartsWith("0") && email.EndsWith("@mybetastore.com")) {
                long intcheck = 0;

                if (phone.Length > 0 && long.TryParse(phone[0], out intcheck)) {
                    return true;
                } else
                    return false;
        ...
        //    if (pi == null)
        //        throw new SimpleMarketException("No property '{0}' found on the instance of type '{1}'.", propertyName, instanceType);
        //    if (!pi.CanWrite)
        //        throw new SimpleMarketException("The property '{0}' on the instance of type '{1}' does not have a setter.", propertyName, instanceType);
        ...
        /// </summary>
        /// <param name="size">Length of Code</param>
        /// <returns></returns>
        ...
        /// </summary>
        public static IDefaultFileProvider DefaultFileProvider { get; set; }
