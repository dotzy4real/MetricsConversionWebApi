...
using MetricsConversion.Domain.Logging;
...
namespace MetricsConversion.Data
{
    public abstract class BaseDataProvider
    {
        ...
        public virtual IDbConnection CreateDbConnection(string connectionString = null) 
        {
            ...
            var appSettings = EngineContext.Current.Resolve<AppInstanceInformationSettings>();

            //LinqToDB.Common.Configuration.AvoidSpecificDataProviderAPI = appSettings.DisplayMiniProfilerInPublicAppInstance;

            return appSettings.DisplayMiniProfilerInPublicAppInstance ? new ProfiledDbConnection((DbConnection)dbConnection, MiniProfiler.Current) : dbConnection;
        }
        ...
        protected void ParseLog<TEntity>(TEntity entity, int entityAction) where TEntity : BaseEntity
        {
            //private readonly IRepository<AuditLog> _auditLogRepository;
            //private readonly IRepository<LogConfiguration> _logConfigRepository;
            //var logger = EngineContext.Current.Resolve<ILogger>();
            IRepository<LogConfiguration> logConfigRepository = EngineContext.Current.Resolve<IRepository<LogConfiguration>>();

            var mLogConfig = from x in logConfigRepository.Table where x.ModelName.ToLower() == typeof(TEntity).Name.ToLower() select x;
            if (mLogConfig.FirstOrDefault() != null && mLogConfig.FirstOrDefault().EnableLog)
            {
                IRepository<AuditLog> _auditLogRepository = EngineContext.Current.Resolve<IRepository<AuditLog>>();
                var mAuditLog = new AuditLog();
                string strAudit = JsonConvert.SerializeObject(entity);
                mAuditLog.ObjectString = strAudit;
                mAuditLog.ActionDate = DateTime.UtcNow;
                mAuditLog.EntityID = entity.Id;
                mAuditLog.EntityType = typeof(TEntity).Name;
                mAuditLog.Action = entityAction;
                _auditLogRepository.Insert(mAuditLog);
        ...
        public TEntity InsertEntityWithAudit<TEntity>(TEntity entity) where TEntity : BaseEntityWithAudit
        {
            using var dataContext = CreateDataConnection();
            entity.Id = dataContext.InsertWithInt32Identity(entity);
            ParseLog(entity, (int)EntityAction.Create);
            return entity;
        }
        ...
        public void UpdateEntity<TEntity>(TEntity entity) where TEntity : BaseEntity
        {
            ...
            ParseLog(entity, (int)EntityAction.Update);
        }
        ...
        /// <param name="entity">Entity with data to update</param>
        /// <typeparam name="TEntity">Entity type</typeparam>
        public void UpdateEntityWithAudit<TEntity>(TEntity entity) where TEntity : BaseEntityWithAudit
        {
            using var dataContext = CreateDataConnection();
            dataContext.Update(entity);
            ParseLog(entity, (int)EntityAction.Update);
        ...
        {
            using var dataContext = CreateDataConnection();
            var command = new CommandInfo(dataContext, sqlStatement);
            var affectedRecords = command.Execute();
            return affectedRecords;
        }
        ...
        protected MappingSchema AdditionalSchema
        {
            get
            {
                ...
                //return new PostgreSQLMappingSchema();
                //Singleton<MappingSchema>.Instance = new PostgreSQLMappingSchema();
